...
**Session 04.5.** **Authentication + Management Access scoping** (security insertion between adapter management and the recon scan). Login page with configurable password (real Pineapple's first-run setup wizard pattern), session-based auth via Flask session cookies + werkzeug scrypt hashing, route protection via `before_request` middleware. Second line of defense: management access deny-list of source CIDRs, blocking specific subnets (e.g. the rogue AP's `10.0.0.0/24` once Phase D lands) at the WSGI layer before auth even runs. New Security tab in Settings with the password-change form + deny-list manager. Console: how the auth.json hash is structured, `werkzeug.security` manual hash generation for emergency recovery, deny-list verification via `curl --interface`.

**Session 04.6.** **Management AP + Wi-Fi client mode for wlan0** (field portability). Pulls Session 18's Networking work forward so the platform is fully UI-controlled — no SSH or Ethernet needed for normal operation. Management AP: `wlan0` hosts a private WPA2 SSID (`PiPineapple-Mgmt` by default on `10.42.0.0/24`) for operator access in the field. Client mode: scan + save + connect to upstream Wi-Fi via the UI. Mode switcher between AP and Client with confirmation prompts. State persisted to `networking.json`, restored on every Flask startup. First real consumers of JobManager — `hostapd` and `dnsmasq` run as managed jobs. Console: hostapd.conf + dnsmasq.conf field-by-field, `nmcli device wifi connect`, the wlan0 mutual-exclusion limit and how to add a 5th adapter to bypass it.
